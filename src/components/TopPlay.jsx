...
const TopPlay = () => {
  ...
  const { data } = useGetTopChartsQuery(3155776842);
  const artistData = useGetTopArtistsQuery()?.data?.data;
        ...
        {artistData?.slice(0, 5).map((artist,index) => (
          ...
          >
            <Link to={`/artists/${artist?.artist?.id}`}>
              <img src={artist?.album?.cover_big} alt="Name" className="rounded-full w-full object-cover" />
            </Link>
          </SwiperSlide>
        ))}
      </Swiper>
